...
- **策略与信号**：加入均线交叉、超跌/偏离阈值、估值分位等信号，形成可配置策略库。
- **回测与评估**：支持区间回测、手续费/滑点模拟、基准对比、胜率与回撤等指标。
- **风险画像**：输出波动率、最大回撤、夏普/索提诺、相关性矩阵与分散度。
- **组合与持仓**：支持组合配置、定投/再平衡、资金曲线与持仓贡献分解。
- **告警与订阅**：阈值预警、信号触发通知（微信/邮件/短信），支持静默与频率控制。
- **数据质量**：缺失/异常检测、补全规则、数据源切换与数据版本审计。
- **对外输出**：报表导出（CSV/Excel/PDF）、开放 API Token、Webhook。
- **权限与安全**：角色权限、只读分享链接、操作审计与访问日志。
